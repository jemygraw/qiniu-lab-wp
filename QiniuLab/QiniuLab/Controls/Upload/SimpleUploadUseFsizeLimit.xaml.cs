...
{
    public partial class SimpleUploadUseFsizeLimit : PhoneApplicationPage
    {
        private Stream uploadFileStream;
        private string uploadFileKey;
        private HttpManager httpManager;
        private string upTokenUrl;
        public SimpleUploadUseFsizeLimit()
        {
            InitializeComponent();
            this.upTokenUrl = string.Format("{0}{1}", Config.API_HOST, Config.SIMPLE_UPLOAD_USE_FSIZE_LIMIT_PATH);
            ...
            httpManager.CompletionHandler = new CompletionHandler(delegate(ResponseInfo getTokenRespInfo, string getTokenResponse)
            {
                if (getTokenRespInfo.StatusCode == 200)
                {
                    ...
                    if (respDict.ContainsKey("uptoken"))
                    {
                        ...
                        writeLog("获取上传凭证...");
                        int fsizeLimit = Convert.ToInt32(respDict["fsizeLimit"]);
                        writeLog("限制大小为: "+fsizeLimit+" 字节");
