...
{
    public partial class ResumableUploadWithoutKey : PhoneApplicationPage
    {
        ...
        public ResumableUploadWithoutKey()
        {
            InitializeComponent();
            ...
            httpManager.CompletionHandler = new CompletionHandler(delegate(ResponseInfo getTokenRespInfo, string getTokenResponse)
            {
                if (getTokenRespInfo.StatusCode == 200)
                {
                    ...
                    if (respDict.ContainsKey("uptoken"))
                    {
                        ...
                        //无key文件上传使用全路径记录文件上传进度
                        string recorderKey = this.uploadFilePath;
                        KeyGenerator keyGen = new KeyGenerator(delegate() { return recorderKey; });
                        new UploadManager(recorder, keyGen).uploadStream(this.uploadFileStream, null, upToken, uploadOptions,
